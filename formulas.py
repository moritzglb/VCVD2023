...
def calc():
    ...
    #implementation of "Rule-of-Thumb" (emergency braking). 
    #Printing the braking distance and overall distance with reaction time of 3 s
    reaction_time = 3 #[s]
    distance_rot = 0.5*(velocity/10)**2
    print ("The emergency brake stopping distance according to the RoT would be",
            distance_rot, "m (including reaction time:", distance_rot+(velocity/10)*reaction_time, "m).")
    print ("")
    ...
    #setting the boundaries for the x-axis to the duration of the braking manoeuvre in s 
    #(start of braking to standstill)
    time = np.linspace(0,stopping_time)
    #setting the boundaries for the x-axis of the distance
    #gradient plot in deg (steepest decline to steepest incline)
